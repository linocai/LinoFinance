...
- `frontend/` Xcode 工程 `LinoFinance.xcodeproj`，**单 target**（2026-06-14 v1 已整删 + 2026-07-10 v3.0.0 P2 widget 又删，见下条）：多平台 App `LinoFinanceV2`（product `LinoF2`，显示名 `LinoF`，源在 `frontend/LinoFinanceV2/`，bundle `com.lino.linofinance`，部署 macOS 26.0/iOS 26.0）。
  ...
  - **widget target `LinoFinanceV2Widgets`（`frontend/LinoFinanceV2Widgets/`，bundle `com.lino.linofinance.widgets`）已于 2026-07-10 v3.0.0 P2 整删**——App Group entitlement 早于 2026-06-17 摘除后 widget 已拿不到实时数据、沦为占位死件，遂删；主 app 喂数代码 `V2WidgetSnapshot.swift`（App/AppModel、Platform/Shared/AppModel+Platform 里的写入调用一并清除）。如需 widget 源码，git tag `v2.5.0`（或更早）可找回。
...
- **删 target（2026-07-10 v3.0.0 P2 删 widget 验证过）**：用每个待删对象自己的 `.remove_from_project`（referrer 计数 cascade），**不要**只清目标对象自己的 to-one/to-many 就收工——序列化按 `project.objects_by_uuid`（一个扁平索引）逐个吐,不是按 rootObject 可达性遍历,所以**多 referrer 的对象必须单独调用 `.remove_from_project`**才能把每个 referrer 都摘干净、referrer 计数真正归零、才会从索引里消失：target 的 `product_reference`（同时也挂在 Products group 的 children 里）、`file_system_synchronized_groups`（同时也挂在主 group 的 children 里）、`build_configuration_list`（要显式删,否则它下头的 Debug/Release `XCBuildConfiguration` 不会被级联清掉,变成孤儿仍写进文件）都要单独删；`PBXProject.attributes["TargetAttributes"][target.uuid]` 是裸 Hash 键、不在对象图里，`remove_from_project` 完全不碰，必须手动 `.delete`；被删 target 若有 Embed App Extensions 类的 copy-files phase 引用它的产物，也要先删对应 `PBXBuildFile`、phase 因此清空了才整删该 phase。全部做完用「存盘后 `File.read` 全文 grep 被删 UUID/target 名必须为空」收口自证。
- 安全流程（已验证零破坏）：改前 `cp project.pbxproj project.pbxproj.bak-<UTC>`；先 no-op open+save round-trip（本项目实测该 round-trip 输出与原文件**逐字节相同**，churn=0；早期笔记写的「churn 仅补空 exceptions=()/删空 packageProductDependencies=()」是历史观察，不代表每次都会发生，仍建议先跑一遍确认基线churn再看真实改动的 diff）+ 临时副本跑脚本 dry-run；落真工程后立即 `xcodebuild -list` + v2 macOS/iOS target 复测 BUILD SUCCEEDED 再继续。**注：临时副本因 SOURCE_ROOT 错位不能直接 build 验源码，只验结构；源码编译验证在真工程原地做。**
...
# 前端（仅 v2 单 target；SwiftPM/swift test 已退役、widget target 已 2026-07-10 删）
...
#   为正式 id（com.lino.linofinance）注册才能签名；注册前本地签名会失败。
...
- 付费 Apple 团队 Team ID `HX73DFL88G`；改 team 要同步 pbxproj 里 2 处 `DEVELOPMENT_TEAM`（v2 app × Debug/Release；**2026-07-10 v3.0.0 P2 删 widget target 后从 4 处收到 2 处**）。真机签名走 automatic signing。
- **bundle id 现状（v1 已删 + 2026-07-10 widget target 已删,只剩 v2 单 app target）**：app = `com.lino.linofinance`（macOS 顶替线上 v1 原地升级；iOS 与已下线的 v1 `.ios` 不同 id → 是新 app）。App Group = `group.com.lino.linofinance` **已于 2026-06-17 从 entitlements 摘除**（commit `b582bb1`，当时 app+widget 两处；widget target 本身已随 P2 整删）——修 macOS 26 每次启动弹「LinoF 想访问其他 App 的数据」（`kTCCServiceSystemPolicyAppData`）：开发签名（`get-task-allow`）+ 本地 ditto 安装下,TCC 对 App Group 共享容器的授权**记不牢、每次重弹**,点「允许」也不持久。**别再把 `application-groups` 加回 entitlements**,除非改走 Developer ID + 公证（稳定身份才能让 TCC 授权持久）。`aps-environment` 暂留 development（真正 Archive 上架时翻 production）。**SIWA/Push capability 须用户在开发者后台为正式 id 注册才能签名**（注册前本地 v2 构建须 `CODE_SIGNING_ALLOWED=NO`）；macOS 已有签好的 Release 装在 `/Applications/LinoF.app`。
- 版本号源（发版统一改）：`backend/pyproject.toml`、`backend/app/core/config.py` 的 `app_version`、`scripts/deploy-api.sh` 的 `EXPECTED_VERSION`、pbxproj 里 4 处（`MARKETING_VERSION` 2 + `CURRENT_PROJECT_VERSION` 2；**2026-07-10 v3.0.0 P2 删 widget target 后从 8 处收到 4 处**）。
