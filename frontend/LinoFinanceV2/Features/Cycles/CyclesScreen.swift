...
// CyclesScreen — D7 周期 (macOS · glass · 分期 + 信用账单周期).
//
// A segmented switcher chooses one of two sections; each renders glass cards
...
//
// v2.5.0 P3: the 订阅 tab was removed (D4=甲). Subscriptions were UI-only here
// — the report's「订阅概览」card (ReportsScreen/ReportsIOSView) and the backend
// subscription endpoints/models remain untouched and fully functional; only
// this tab's CRUD surface is gone.
struct CyclesScreen: View {
    @ObservedObject var model: AppModel
    @StateObject private var cyclesModel: CyclesModel

    enum Section: String, CaseIterable, Identifiable {
        case installments, statements
    ...
    @State private var section: Section = .installments
