...
- **`POST /ai/plans` is idempotent over a short window for sequential resubmits (v3.1.0 P1).** The server computes a `content_fingerprint` = `sha256(source_text.strip() + "\x1e" + canonical(actions))` where `canonical(actions)` is the compact, `sort_keys` JSON of the **request-original** `[{action_type, payload}, …]` the client sent (`explanation` / `confidence` excluded; the pure-text path with no client actions hashes an empty `[]`). If a plan with the same fingerprint was created within the last **120 seconds**, the endpoint returns that **existing plan with HTTP `200`** (no new plan, no LLM call, no new actions/audit); a first-of-its-kind request returns **`201`**. Both responses share the identical `AIPlanRead` body shape, so a client can treat them transparently. The window catches mechanical repeats (double-tap, Siri retry, Shortcut loop, `prepareExecution` resubmit) but not a genuinely repeated purchase hours later (time pulls those into distinct plans). Fingerprints are hashed **before** any model call, so dedup also short-circuits the LLM. The dedup hit returns the existing plan **regardless of its status** (even `executed` / `rejected`): the execute state gate then rejects a second `execute` (`400`), making a **sequential** same-content-create-then-execute flow idempotent (a resubmit that arrives after the first request already committed always sees and reuses that same plan). This moves the "at most one executable plan" invariant from a best-effort client-side reject to server-side dedup, root-causing the v3.0.0 review's resubmit-then-reject orphan-plan double-execute (重要-3). **This is not a concurrency guarantee** (v3.1.0 评审 建议-1: softened from an earlier "idempotent end-to-end" claim that overstated it) — the query→LLM→insert path is not one atomic transaction, so two requests racing inside the same 120s window can both miss the fingerprint lookup and each insert their own plan; there is no advisory lock, and the `content_fingerprint` index is deliberately non-unique. The residual window is accepted as-is, not closed: a single user's own retries are effectively sequential (Siri is modal; a double-tap/Back Tap resolves to one shortcut invocation), and a genuine double write stays recoverable via rollback plus the spoken/notified confirmation every write already surfaces. The `content_fingerprint` column is nullable (historical rows are `NULL`) with a non-unique index — the window semantics live in the query, not a DB constraint.
...
- **Automatic daily exchange rate (v3.0.0 P6, §3/§5.8 D3=甲).** `app.services.exchange_rate_auto.fetch_daily_auto_rates` (CLI entry `scripts/fetch_exchange_rates.py`, meant to run off the `linofinance-jobs` systemd timer — see `docs/deployment.md`) pulls the free, keyless `open.er-api.com` API once per non-CNY currency LinoFinance supports and inserts a `CurrencyRate(source="auto")` row for `app_today()` (business timezone) — **only when the day has no rate at all yet, manual or auto**; it never edits or overwrites an existing row, so a manual entry always wins permanently and a re-run on the same day is a no-op. The request uses `base=<non-CNY currency>` (e.g. `open.er-api.com/v6/latest/USD`) and reads `rates[CNY]` directly (that base-currency framing already gives "1 unit of the currency = N CNY" with no inversion needed). Network/timeout/non-2xx/malformed-body/missing-or-non-positive-rate failures are all swallowed and logged as a warning — this path never raises and never blocks a caller. This is a background job, not an HTTP endpoint; no new route was added. Zero migration — `currency_rates.source` already existed with `default="manual"`, and `"auto"` is simply a new value written directly by this service (the `POST /currency-rates` request schema's `source` pattern is unchanged and user-facing rate creation still cannot claim `source="auto"`).
