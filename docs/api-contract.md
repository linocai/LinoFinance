...
- **`POST /ai/plans` is idempotent over a short window (v3.1.0 P1).** The server computes a `content_fingerprint` = `sha256(source_text.strip() + "\x1e" + canonical(actions))` where `canonical(actions)` is the compact, `sort_keys` JSON of the **request-original** `[{action_type, payload}, …]` the client sent (`explanation` / `confidence` excluded; the pure-text path with no client actions hashes an empty `[]`). If a plan with the same fingerprint was created within the last **120 seconds**, the endpoint returns that **existing plan with HTTP `200`** (no new plan, no LLM call, no new actions/audit); a first-of-its-kind request returns **`201`**. Both responses share the identical `AIPlanRead` body shape, so a client can treat them transparently. The window catches mechanical repeats (double-tap, Siri retry, Shortcut loop, `prepareExecution` resubmit) but not a genuinely repeated purchase hours later (time pulls those into distinct plans). Fingerprints are hashed **before** any model call, so dedup also short-circuits the LLM. The dedup hit returns the existing plan **regardless of its status** (even `executed` / `rejected`): the execute state gate then rejects a second `execute` (`400`), making the same-content-create-then-execute flow idempotent end-to-end. This moves the "at most one executable plan" invariant from a best-effort client-side reject to server-side dedup, root-causing the v3.0.0 review's resubmit-then-reject orphan-plan double-execute (重要-3). The `content_fingerprint` column is nullable (historical rows are `NULL`) with a non-unique index — the window semantics live in the query, not a DB constraint.
