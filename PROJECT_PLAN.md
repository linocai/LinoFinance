...
**v1.3.0（审计修复版）代码施工完成（2026-06-10），所有门禁通过，未发布未部署。** 基于 v1.2.0 全量审计（`archive/REVIEW_REPORT_v1.2.0.md`），范围 = 致命 4 + 重要 15 + 文档归真。12 个 Phase（P0–P12）全部落在本地 `release/v1.3.0`（从 v1.2.0 的 `50e2188` 切出）。门禁：backend pytest **155 通过**、ruff 干净、alembic 单一 head `202606100002`；`swift test` **16 通过**；xcodebuild macOS + iOS Simulator BUILD SUCCEEDED；`deploy-api.sh --dry-run` 干净（期望 ver `1.3.0`）；三项 curl smoke 全过。**发布 / tag / push / merge 入 `main` / live 部署 / 真机均为用户侧手动**（见 §6「用户侧收尾（v1.3.0）」），生产仍运行 v1.2.0。完整 Plan + 实施记录见 `archive/v1.3.0_fix_plan.md`，摘要见下半部 v1.3.0 变更日志。

## 5. 当前版本 Plan

> v1.3.0 已完成施工（2026-06-10），全文（§5.0–§5.2 完整 Plan + 实施记录）见 `archive/v1.3.0_fix_plan.md`。发布 / tag / push / live 部署 / 真机为用户侧手动，见 §6「用户侧收尾（v1.3.0）」。本节回到占位空节，下一版 @planner 在此重新填写。
...
### 用户侧收尾（v1.3.0，待用户手动执行，含 v1.2.0 遗留合并）

> v1.3.0 代码施工已完成、全门禁通过，**未发布未部署**。以下全部为用户手动（git / live / 真机），builder 不碰。

发版 / 部署：

1. 审阅施工结果后：合并 `release/v1.3.0` 入 `main`（PR）、本地打 tag `v1.3.0`、push（全部用户手动，git 纪律）。
2. live 部署：`scripts/deploy-api.sh`（先 `--dry-run`，已确认干净，期望 ver `1.3.0`）；更新 `/etc/linofinance/api.env`：`LINOFINANCE_APP_TIMEZONE=Asia/Shanghai`（可选 `LINOFINANCE_APPLE_SUB_ALLOWLIST`）；安装并 `systemctl enable --now linofinance-jobs.timer`（不装则 T-5/3/1/0 还款提醒永不触发，见 deployment.md）。
3. 生产闸门确认：psql 查 `SELECT id, apple_user_id, disabled FROM users;` 确认仅本人且激活；若表为空，**部署后尽快本人完成一次 Apple 登录占首用户位**。

真机验证（单测抓不到，v1.3.0 新增 + v1.2.0 遗留合并执行）：

4. 本人 Apple 登录闭环仍正常；第二个 Apple ID 登录被拒（可选）；手动 disable 一个测试用户后其旧会话立刻 401；还款提醒真推送（等 T-N 触发或在 hz 手跑 `run_scheduled_jobs.py`）。
5. macOS 用 `ditto` 换装 `/Applications/LinoF.app`（旧包备份 `LinoF.app.bak-<UTC>`）；iOS 真机签名 install（automatic signing，Team ID `HX73DFL88G`，验证 portal capability/profile）。
6. v1.2.0 遗留三项：真机 Apple ID 完整登录闭环；真机收 APNs 推送（含 hz 上 push smoke）；iOS 真机签名 install（与第 5 条合并执行）。
...
## v1.3.0 — 2026-06-10 · 审计修复版（致命 4 + 重要 15 + 文档归真）

主题：v1.2.0 全量审计的纯缺陷修复——单人闸门、报销 settle 旁路封堵、前端金额解析收口、时区闭环、账本口径修正、主数据 PATCH、限流/导出/附件防护、还款提醒链路、客户端缺陷批修、SPM 名实归位、文档归真。无新功能。**代码施工完成，所有门禁通过，未发布未部署**（生产仍 v1.2.0；tag/push/merge/live/真机为用户侧手动，见 §6 用户侧收尾）。
